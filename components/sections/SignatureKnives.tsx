...
export function SignatureKnivesSection({ pieces }: SignatureKnivesSectionProps) {
  ...
  return (
    <section
      className="sticky top-0 z-30 w-full h-[var(--app-height)] overflow-hidden bg-background"
      data-stack-section
    >
      <div className="flex h-full min-h-0 flex-col lg:grid lg:grid-rows-1 lg:grid-cols-2 lg:h-full">
        <div className="order-2 lg:order-1 relative flex-1 min-h-0 w-full overflow-hidden bg-secondary/30">
          <Image
            src={activeImageSrc}
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between text-neutral-100">
            <span className="text-[0.65rem] uppercase tracking-[0.28em]">
              {activeKnife?.title}
            </span>

            {activePrice && (
              <span className="text-[0.7rem] tracking-normal tabular-nums font-medium">
                {activePrice}€
              </span>
            )}
          </div>
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className="text-base font-light whitespace-nowrap tabular-nums tracking-normal">
